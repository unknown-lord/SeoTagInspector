...
export default function GooglePreview({ title, description, url }: GooglePreviewProps) {
  ...
  return (
    <Card data-testid="card-google-preview">
      ...
      <CardContent>
        <div className="space-y-1 p-4 bg-muted/30 rounded-md">
          <div className="text-sm text-muted-foreground flex items-baseline gap-1">
            <span className="font-medium">{domain}</span>
            {path && <span className="text-xs">{path}</span>}
          </div>
          <h3 className="text-xl text-primary font-normal hover:underline cursor-pointer" data-testid="text-preview-title">
            {title || "Untitled Page"}
          </h3>
